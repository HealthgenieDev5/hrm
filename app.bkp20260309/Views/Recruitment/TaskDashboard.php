...
<style>
    .summary-card .card-body { padding: 1.25rem 1.5rem; }
    .summary-card .summary-value { font-size: 2rem; font-weight: 700; line-height: 1; }
    .summary-card .summary-label { font-size: 0.8rem; text-transform: uppercase; letter-spacing: .05em; }
    .filter-section label { font-size: 0.85rem; font-weight: 600; }
    .assignee-line { display: flex; align-items: center; gap: 6px; margin-bottom: 3px; font-size: 0.82rem; }
    .task-action-btn { margin-bottom: 2px; }
    #task-dashboard-table td { vertical-align: middle; }
    .overdue-date { color: #f1416c; font-weight: 600; }
</style>
...
(function() {
    ...
    function renderActions(row) {
        if (IS_MANAGER) {
            ...
            const myAssignee = row.assignees
                ? row.assignees.find(function(a) { return parseInt(a.assigned_to) === CURRENT_USER_ID; })
                : null;
            let html = '';
    ...
    const table = $('#task-dashboard-table').DataTable({
        ...
        columns:  [
            ...
            {
                data:   null,
                render: function(data, type, row) { return renderAssignees(row); },
            },
            {
                data:   'due_date',
                render: function(data, type, row) {
        ...
        order:      [[4, 'asc']],
        pageLength: 25,
        language:   { emptyTable: 'No tasks found for the selected filters.' },
    });
    ...
    function loadDashboardTasks() {
        const statuses   = $('.filter-status:checked').map(function() { return this.value; }).get().join(',');
    ...
    function loadHrEmployees(callback) {
        $.ajax({
            ...
            dataType: 'json',
            success:  function(resp) {
                if (resp.success && resp.data) { callback(resp.data); }
            },
        });
    }
    ...
    $('#btn-apply-filters').on('click', function() { loadDashboardTasks(); });
    ...
    $('#btn-clear-filters').on('click', function() {
        ...
        if (IS_MANAGER) { $('#filter-assigned-to').val(null).trigger('change'); }
    ...
    if (IS_MANAGER) {
        ...
        $('#confirm-create-task').on('click', function() {
            ...
            if (!jobId)                             { Swal.fire('Error', 'Please select a job listing.', 'error'); return; }
            if (!taskType)                          { Swal.fire('Error', 'Please select a task type.', 'error'); return; }
            if (!assignTo || !assignTo.length)      { Swal.fire('Error', 'Please select at least one assignee.', 'error'); return; }
            if (!dueDate)                           { Swal.fire('Error', 'Please select a due date.', 'error'); return; }
        ...
        $(document).on('click', '.edit-task-btn', function() {
            ...
            const rowData = table.rows(function(idx, data) { return data.task_id === taskId; }).data()[0];
            if (!rowData) return;
        ...
        $('#confirm-reassign').on('click', function() {
            ...
            if (!assigneeRecordId) { Swal.fire('Error', 'Please select an assignee to reassign.', 'error'); return; }
            if (!newAssignedTo)    { Swal.fire('Error', 'Please select a new HR member.', 'error'); return; }

            $.ajax({
                url:      BASE_URL + 'recruitment/job-listing/tasks/reassign',
                type:     'POST',
                data:     { assignee_record_id: assigneeRecordId, new_assigned_to: newAssignedTo },
                dataType: 'json',
                success:  function(resp) {
    ...
    $('#confirm-update-status').on('click', function() {
        ...
        $.ajax({
            ...
            type:     'POST',
            data:     { assignee_record_id: assigneeId, status: newStatus },
            dataType: 'json',
            success:  function(resp) {
