                    ...
                    <table id="job_listings_table" class="table table-sm table-hover table-striped table-row-bordered" style="width: 100% !important;">
                        <thead class="bg-white">
                            <tr>
                                ...
                                <th class="text-center" style="min-width: 100px;"><strong>HOD</strong></th>
                                ...
                                <th class="text-center"><strong>Qualification</strong></th>
                                ...
                                <th class="text-center" style="min-width: 100px;"><strong>Shift Timing</strong></th>
...
<script>
    const currentEmployeeId = <?= json_encode(session()->get('current_user')['employee_id'] ?? null) ?>;
    $(document).ready(function() {
        ...
        $('#job_listings_table tbody').on('click', '.close-job-listing', function(e) {
            e.preventDefault();
        ...
        var table = $("#job_listings_table").DataTable({
            ...
            "columns": [{
                ...
                },
                {
                    data: null,
                    render: function(data, type, row, meta) {
                        const hasCreator = row.created_by !== null && row.created_by !== undefined;
                        const hasCurrentUser = currentEmployeeId !== null && currentEmployeeId !== undefined;
                        const canEdit = hasCreator && hasCurrentUser && String(row.created_by) === String(currentEmployeeId) && !row.approved_by_hr_manager;
                        const canDownloadPDF = row.approved_by_hr_manager;
                        const editButton = canEdit ? `<a href="<?= site_url('recruitment/job-listing/edit/') ?>${row.id}" class="btn btn-light-warning btn-active-warning btn-sm" data-bs-toggle="tooltip" title="Edit Job">
                                <span class="fa fa-edit"></span>
                            </a>` : '';
                        const downloadButton = canDownloadPDF ? `<a href="<?= site_url('recruitment/job-listing/download-job-opening-pdf/') ?>${row.id}" class="btn btn-light-primary btn-active-primary btn-sm" data-bs-toggle="tooltip" title="Download PDF">
