...
class ProcessorHelper
{
    ...
    public static function get_punch_time_including_od($in_time__Raw, $out_time__Raw, $shift_start__Raw, $shift_end__Raw, $date, $employee_id)
    {
        ...
        //     if punched out before shift end then the early leaving will be 
    ...
    public static function get_time_difference($start_time__Raw, $end_time__Raw, $returnType = 'hours')
    {
        if (!empty($start_time__Raw) && !empty($end_time__Raw) && $start_time__Raw !== '--:--' && $end_time__Raw !== '--:--') {
    ...
    public static function get_shift_type($shiftId)
    {
        if (empty($shiftId)) {
            return null;
        }
        $ShiftModel = new ShiftModel();
        $ShiftData = $ShiftModel->find($shiftId);
        return $ShiftData['shift_type'];
    }
    ...
    public static function twoByThree(float $value)
    {
        return (float) $value * 2 / 3; # return 2 third of actual value%
    }
    ...
    public static function getLeaveBalance($employee_id, $show_used_rh = false)
    {
        ...
        $compOffDaysBalance = self::getCompOffBalanceDays((int)$employee_id, 90);
        ...
        foreach ($LeaveBalanceNew as $i => $BalanceNewRow) {
            if ($BalanceNewRow['leave_code'] == 'COMP OFF') {
                ...
                $LeaveBalanceNew[$i]['balance'] = $compOffDaysBalance;
            }
        }

        // print_r([
        //     'comp_off_from' => $comp_off_from,
        //     'comp_off_to' => $comp_off_to,
        //     'BalanceCompOff' => $BalanceCompOff,
        //     'FinalCompOffBalance' => $FinalCompOffBalance,
        // ]);
        // die();
        ...
        */
        $compOffMinutesBalance = self::getCompOffBalanceMinutes((int)$employee_id, 90);
        ...
        foreach ($LeaveBalanceNew as $i => $BalanceNewRow) {
            if ($BalanceNewRow['leave_code'] == 'COMP OFF Minutes') {
                ...
                $LeaveBalanceNew[$i]['balance'] = $compOffMinutesBalance <= 0 ? '00:00' : self::convertToTime($compOffMinutesBalance);
            }
        }
        ####################End::Update FinalCompOffBalance####################
        ####################End::CompOff Minutes####################
    ...
    /**
     * Calculates Comp-Off balance (days) using FIFO allocation within expiry window.
     * - Credits: comp_off_credit (approved)
     * - Debits: leave_requests (pending + approved) for "COMP OFF"
     *
     * Rules:
     * - Each credit expires after $expiryDays days from working_date
     * - A debit consumes only credits that were NOT expired on the debit date
     * - Result is sum of remaining credits that are NOT expired as of today
     */
    public static function getCompOffBalanceDays(int $employeeId, int $expiryDays = 90): float
    {
        $today = new \DateTimeImmutable(date('Y-m-d'));

        // 1) Load all APPROVED credits up to today (oldest first)
        $CompOffCreditModel = new CompOffCreditModel();
        $creditsRaw = $CompOffCreditModel
            ->select('working_date, exchange')
            ->where('employee_id', $employeeId)
            ->where('status', 'approved')
            ->where('working_date <=', $today->format('Y-m-d'))
            ->orderBy('working_date', 'ASC')
            ->findAll();

        // Convert to "credit buckets"
        $credits = [];
        foreach ($creditsRaw as $row) {
            $earnedOn = new \DateTimeImmutable($row['working_date']);
            $expiresOn = $earnedOn->modify("+{$expiryDays} days");

            $credits[] = [
                'earned_on'  => $earnedOn,
                'expires_on' => $expiresOn,
                'remaining'  => (float) $row['exchange'], // usually 0.5 or 1
            ];
        }
        ...
        // 2) Load all debits (pending + approved) up to today (oldest first)
        // NOTE: use from_date as the "consume date" (date leave is taken)
        $LeaveRequestsModel = new LeaveRequestsModel();
        $debitsRaw = $LeaveRequestsModel
            ->select('from_date, to_date, number_of_days, status')
            ->where('employee_id', $employeeId)
            ->where('type_of_leave', 'COMP OFF')
            ->whereIn('status', ['pending', 'approved'])
            ->where('from_date <=', $today->format('Y-m-d'))
            ->orderBy('from_date', 'ASC')
            ->findAll();

        // 3) Allocate debits against credits (FIFO) using debit-date validity
        foreach ($debitsRaw as $d) {
            $debitDate = new \DateTimeImmutable($d['from_date']);
            $need = (float) $d['number_of_days'];

            if ($need <= 0) {
                continue;
            }

            // consume from oldest valid credit first
            for ($i = 0; $i < count($credits) && $need > 0; $i++) {
                if ($credits[$i]['remaining'] <= 0) {
                    continue;
                }

                // credit must be valid on debit date
                // valid if earned_on <= debitDate AND expires_on >= debitDate
                if ($credits[$i]['earned_on'] > $debitDate) {
                    // since credits are sorted by earned_on ASC,
                    // if this one is in future of debitDate, next ones also will be.
                    break;
                }

                if ($credits[$i]['expires_on'] < $debitDate) {
                    // expired before this debit date; cannot use it for this debit
                    continue;
                }

                $take = min($credits[$i]['remaining'], $need);
                $credits[$i]['remaining'] -= $take;
                $need -= $take;
            }

            // If $need is still > 0 here, it means system has over-utilization.
            // We do not return negative balance; we just can't consume further.
        }

        // 4) Sum remaining credits that are NOT expired today
        $balance = 0.0;
        foreach ($credits as $c) {
            ...
            if ($c['expires_on'] < $today) {
                continue; // expired today => not count in final balance
            }
            $balance += $c['remaining'];
        }
    ...
    public static function getCompOffBalanceMinutes(int $employeeId, int $expiryDays = 90): int
    {
        $today = new \DateTimeImmutable(date('Y-m-d'));
        ...
        foreach ($creditsRaw as $row) {
            ...
            $credits[] = [
                'earned_on'  => $earnedOn,
                'expires_on' => $expiresOn,
                'remaining'  => (int) self::convertToMinutes($row['minutes']), // uses your existing helper
            ];
        }
        ...
        // Debits (utilized minutes) — assuming 'date' is the consume date
        $CompOffMinutesUtilizedModel = new CompOffMinutesUtilizedModel();
        $debitsRaw = $CompOffMinutesUtilizedModel
            ->select('date, minutes')
            ->where('employee_id', $employeeId)
            ->where('type', 'utilized')
        ...
        foreach ($debitsRaw as $d) {
            $debitDate = new \DateTimeImmutable($d['date']);
            $need = (int) $d['minutes'];

            if ($need <= 0) continue;

            for ($i = 0; $i < count($credits) && $need > 0; $i++) {
                if ($credits[$i]['remaining'] <= 0) continue;
        ...
        foreach ($credits as $c) {
            ...
            if ($c['expires_on'] < $today) continue;
            $balance += $c['remaining'];
        }
