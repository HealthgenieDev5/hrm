...
if( isset($_REQUEST) && !empty($_REQUEST) ){
	...
	if( !empty($employee_id) && !empty($year) ){
		...
		print_r( addslashes($_REQUEST['appr_performance_history_response'][1]) );
		echo '</pre>';*/
		...
		if( isset($_REQUEST['appr_performance_history_response']) && !empty($_REQUEST['appr_performance_history_response']) ){
			foreach( $_REQUEST['appr_performance_history_response'] as $parameter_id => $response){
				...
				if(mysqli_num_rows($find_existing_in_appr_performance_history_response_query) == 1){
					...
					$update_in_appr_performance_history_response_sql = "update appr_performance_history_response set response = '".addslashes($response)."' where id = '".$appr_performance_history_response_id."'";

					/*echo '<pre id="abc" style="margin-top:100px">';
					print_r( $update_in_appr_performance_history_response_sql );
					echo '</pre>';*/
					...
					$insert_in_appr_performance_history_response_sql = "insert into appr_performance_history_response (employee_id, parameter_id, response, year) values ('".$employee_id."', '".$parameter_id."', '".addslashes($response)."', '".$year."')";
		...
		if( isset($_REQUEST['appr_questions_response']) && !empty($_REQUEST['appr_questions_response']) ){
			foreach( $_REQUEST['appr_questions_response'] as $question_id => $answer){
				...
				if(mysqli_num_rows($find_existing_in_appr_questions_response_query) == 1){
					...
					$update_in_appr_questions_response_sql = "update appr_questions_response set answer = '".addslashes($answer)."' where id = '".$appr_questions_response_id."'";
					...
					$insert_in_appr_questions_response_sql = "insert into appr_questions_response (employee_id, question_id, answer, year) values ('".$employee_id."', '".$question_id."', '".addslashes($answer)."', '".$year."')";
		...
		##################appr_kra_response##################
		if( isset($_REQUEST['appr_kra_self_rating']) && !empty($_REQUEST['appr_kra_self_rating']) ){
			foreach( $_REQUEST['appr_kra_self_rating'] as $accountability_id => $self_rating){
				if( $self_rating == '' ){
					$self_rating = 'NULL';
				}
				...
				if(mysqli_num_rows($find_existing_in_appr_kra_response_query) == 1){
					...
					$update_in_appr_kra_response_sql = "update appr_kra_response set self_rating = '".$self_rating."' where id = '".$appr_kra_response_id."'";
					...
					$insert_in_appr_kra_response_sql = "insert into appr_kra_response (employee_id, accountability_id, self_rating, year) values ('".$employee_id."', '".$accountability_id."', ".$self_rating.", '".$year."')";
		...
		if( isset($_REQUEST['personal_trait_self_rating']) && !empty($_REQUEST['personal_trait_self_rating']) ){
			foreach( $_REQUEST['personal_trait_self_rating'] as $personal_trait_id => $self_rating){
				if( $self_rating == '' ){
					$self_rating = 'NULL';
				}
				...
				if(mysqli_num_rows($find_existing_in_appr_employee_personal_traits_reponse_query) == 1){
					...
					$update_in_appr_employee_personal_traits_reponse_sql = "update appr_employee_personal_traits_reponse set self_rating = '".$self_rating."' where id = '".$appr_employee_personal_traits_reponse_id."'";
					...
					$insert_in_appr_employee_personal_traits_reponse_sql = "insert into appr_employee_personal_traits_reponse (employee_id, personal_trait_id, self_rating, year) values ('".$employee_id."', '".$personal_trait_id."', ".$self_rating.", '".$year."')";
...
<!--end::Custom Css-->
<?php 
include_once("./inc/header-bottom.php");
?>

<?php 
/*if( $_SESSION['login']['user_role'] == 'hr' ){
	include_once("./inc/page-header-hr.php");
}else{
...
<?php
include_once("./enabled-departments.php");
#begin:check if the appraisal is enabled for this department
if( !in_array( $current_employee_data['department_id'], $enabled_departments ) ){
	?>
	<div class="section">
		<div class="container">
			<div class="card mt-5">
				<div class="card-body p-3">
					<h3 class="text-center my-5 py-3">Appraisal is not initiated for your department</h3>
				</div>
			</div>
		</div>
	</div>
...
?>
	<!-- <div class="section">
		<div class="container">
			<div class="card mt-5">
				<div class="card-body p-3">
					<h3 class="text-center my-5 py-3"></h3>
					<script>
			    	...
			    	<div class="row m-0">
			    		<div class="col-xl-4 left-side d-flex flex-column flex-grow-1 py-4 px-4">
			    			<div class="text mb-3 border-bottom">
		                        ...
		                        <p class="text-white fst-italic" style="opacity: 0.7;">Please verify your personal information.<br><small style="font-size: 12px; line-height: 1.25; display: block;">In case the information is not correct, please contact HR</small></p>
			    			...
			    			<div class="flex-grow-1">
		                    	...
		                    	<div class="row mb-2">
			                    	...
			                    	<!-- <input type="hidden" id="overall_rating" name="overall_rating" value="<?php echo $current_employee_data['overall_rating']; ?>" />
			                    	<input type="hidden" id="remarks" name="remarks" value="<?php echo $current_employee_data['remarks']; ?>" /> -->
			                    </div>
		                    </div>
			    		</div>
			    		<div class="col-xl-8 right-side d-flex flex-column flex-grow-1 py-4 px-0">
			    			...
			    			<ul id="progressbar" class="px-0 d-flex">
                                <li class="text-center active" id="performance">
                                	<strong>Performance</strong>
                                </li>
                                <li id="questions" class="text-center <?php if($form_submitted == true){ echo 'active'; } ?>">
                                	<strong>Questions</strong>
                                </li>
                                <li id="kra" class="text-center <?php if($form_submitted == true){ echo 'active'; } ?>">
                                	<strong>KRA</strong>
                                </li>
                                <li id="personal_traits" class="text-center <?php if($form_submitted == true){ echo 'active'; } ?>">
                                	<strong>Personal Traits</strong>
                                </li>
                                <li id="confirm" class="text-center <?php if($form_submitted == true){ echo 'active'; } ?>">
                            ...
                            <fieldset class="flex-grow-1 flex-column px-5 form-step" data-title="Performance" data-description="This section should be completed by Employee before the conclusion of the appraisal" style="<?php if($form_submitted == true){ echo 'display: none;'; } ?>" >
                            	<div class="d-flex flex-column flex-grow-1 mb-5">
									...
									$appr_performance_history_master_num_rows = mysqli_num_rows($appr_performance_history_master_query);
									if( $appr_performance_history_master_num_rows > 0 ){
										while( $row = mysqli_fetch_assoc($appr_performance_history_master_query) ){
											...
											<div class="row mb-3">
												...
												<div class="col-md-8 d-flex align-items-center">
													<input class="form-control flex-grow-1 rounded-0 bg-transparent" style="border: none; border-bottom: 1px dotted var(--bs-secondary); box-shadow: none;" type="text" id="appr_performance_history_response_<?php echo $row['id']; ?>" name="appr_performance_history_response[<?php echo $row['id']; ?>]" value="<?php echo htmlentities($row['response']); ?>">
												</div>
											</div>
											<?php
										}
									...
									if( mysqli_num_rows($appr_questions_master_query) > 0 ){
										...
										while( $row = mysqli_fetch_assoc($appr_questions_master_query) ){
											...
											<div class="row mb-3">
												...
												<div class="col-md-12 d-flex align-items-center">
													<textarea class="form-control" id="appr_questions_response_<?php echo $row['id']; ?>" name="appr_questions_response[<?php echo $row['id']; ?>]"><?php echo htmlentities($row['answer']); ?></textarea>
												</div>
											</div>
											<?php
										}
									...
									if( mysqli_num_rows($appr_kra_master_query) > 0 ){
										...
										while( $row = mysqli_fetch_assoc($appr_kra_master_query) ){
											...
											<tr>
												...
												<td>
													<input type="number" class="rating text-warning" id="appr_kra_self_rating_<?php echo $row['id']; ?>" name="appr_kra_self_rating[<?php echo $row['id']; ?>]"  data-clearable="remove" data-inline=false value="<?php echo $row['self_rating']; ?>"/>
												</td>
												<td>
													<input type="number" class="rating text-secondary" id="appr_kra_hod_rating_<?php echo $row['id']; ?>" name="appr_kra_hod_rating[<?php echo $row['id']; ?>]" data-clearable="remove" data-inline=false value="<?php echo $row['hod_rating']; ?>" data-readonly=true disabled/>
												</td>
											</tr>
											<?php
										}
									...
									if( mysqli_num_rows($appr_employee_personal_traits_master_query) > 0 ){
										...
										while( $row = mysqli_fetch_assoc($appr_employee_personal_traits_master_query) ){
											...
											<tr>
												...
												<td>
													<input type="number" class="rating text-warning" id="personal_trait_self_rating_<?php echo $row['id']; ?>" name="personal_trait_self_rating[<?php echo $row['id']; ?>]" data-clearable="remove" data-inline=false value="<?php echo $row['self_rating']; ?>" />
												</td>
												<td>
													<input type="number" class="rating text-secondary" id="personal_trait_reporting_manager_rating_<?php echo $row['id']; ?>" name="personal_trait_reporting_manager_rating[<?php echo $row['id']; ?>]" data-clearable="remove" data-inline=false value="<?php echo $row['reporting_manager_rating']; ?>" data-readonly=true disabled />
												</td>
												<td>
													<input type="number" class="rating text-secondary" id="personal_trait_hod_rating_<?php echo $row['id']; ?>" name="personal_trait_hod_rating[<?php echo $row['id']; ?>]" data-clearable="remove" data-inline=false value="<?php echo $row['hod_rating']; ?>" data-readonly=true disabled />
												</td>
											</tr>
											<?php
										}
...
#end:check if the appraisal is enabled for this department
?>
</main>
<!-- End #main -->
