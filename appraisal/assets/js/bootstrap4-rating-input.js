(function ($) {
    ...
    function createRatingEl($input, options) {
        ...
        if (options.inline) {
            ...
            var $ratingEl = $('<div class="rating-input d-flex align-items-center"></div>');
        }
        ...
        for (var i = options.min; i <= options.max; i++) {
            $ratingEl.append('<i class="' + options.iconLib + '" data-value="' + i + '" title="' + i + '" style="padding-right:2px; cursor:pointer;"></i>');
        }
                ...
                if (!rating.options.readonly) {
                    rating.$el
                        .on('mouseenter', starSelector(), function () {
                            ...
                            // alert( rating.options.readonly );
                            $(this).css({'position':'relative'});
                            $(this).html('<span class="my-value-tooltip rounded-3 bg-dark text-white position-absolute" style="top:0px; z-index:1">'+$(this).data('value')+'</span>');
                        })
