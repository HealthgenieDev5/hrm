...
class LoyaltyIncentive extends BaseController
{
	...
	public function index($id)
	{
		...
		if (($data['enable_lwf'] == 'yes')) {
			$data['lwf_employee_contribution'] = ((($data['monthly_gross_salary'] * 0.2) / 100 <= 31) ? ($data['monthly_gross_salary'] * 0.2) / 100 : 31);
		...
		// return view('Master/EmployeeLoyaltyIncentive', $data); die();

		$loyalty_incentive_from = $data['loyalty_incentive_from'] ?? '';
		$loyalty_incentive_to = $data['loyalty_incentive_to'] ?? '';
		if (!empty($loyalty_incentive_from) && !empty($loyalty_incentive_to)) {
			$data['loyalty_incentive_years'] = $this->getYearMonthDifference($loyalty_incentive_from, $loyalty_incentive_to);
			// echo $data['loyalty_incentive_years'];
			// die();
		} else {
			return view('User/Unauthorised', ["page_title" => "Incomplete data", "page_content" => "Start and End dates are required for Loyalty Incentive<br>Please update salary data then try again."]);
			die();
		}
		...
		$content = view('Master/EmployeeLoyaltyIncentive', $data);
		$dompdf->loadHtml($content);
		$dompdf->setPaper('A4', 'portrait');
		$dompdf->render();
		$dompdf->stream("loyalty-incentive-" . strtolower($data['first_name'] . '-' . $data['last_name']) . "-.pdf");
	}

	private function getYearMonthDifference($date1, $date2)
	{
		...
		$yearsInWords = $this->numberToWords($years);
		$monthsInWords = $this->numberToWords($months);

		if ($years > 0 && $months > 0) {
			return "$years ($yearsInWords) Years and $months ($monthsInWords) Months";
		} elseif ($years > 0) {
			return "$years ($yearsInWords) Years";
		} elseif ($months > 0) {
			return "$months ($monthsInWords) Months";
		} else {
			return "0 months";
		}
	}
