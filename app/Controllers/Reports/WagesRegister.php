...
class WagesRegister extends BaseController
{
    public $session;
    ...
    public function index()
    {
        ...
        $month_request = $_REQUEST['month_request'] ?? date('Y-m', strtotime(first_date_of_last_month()));
        ...
        return view('Reports/WagesRegister', $data);
    }
    ...
    private function getAttendanceRegisterData($employees, $month)
    {
        ...
        $dompdf->render();
        $dompdf->stream("AttendanceRegister-" . $data['month'] . ".pdf");
    }
    ...
    private function getWagesRegisterData($employees, $month)
    {
        ...
        foreach ($employees as $index => $employee) {
            ...
            $record = $PreFinalSalaryModel
                ...
                ->where('pre_final_salary.month', date('n', strtotime($month)))
                ->first();
        ...
        $dompdf->render();
        $dompdf->stream("WagesRegister-" . $data['month'] . ".pdf");
    }
    ...
    private function getWageSlipData($employees, $month)
    {
        ...
        $dompdf->setPaper('A4', 'portrait');
        $dompdf->render();
        $dompdf->stream("WageSlip-" . $data['month'] . ".pdf");
    }
    ...
    private function getOvertimeRegisterData($employees, $month)
    {
        ...
        $dompdf->setPaper('A4', 'portrait');
        $dompdf->render();
        $dompdf->stream("OvertimeRegister-" . $data['month'] . ".pdf");
    }
    ...
    private function getMusterRollData($employees, $month)
    {
        ...
        $dompdf->setPaper('legal', 'landscape');
        $dompdf->render();
        $dompdf->stream("MusterRoll-" . $data['month'] . ".pdf");
    }
    ...
    private function getLeaveRegisterData($employees, $month)
    {
        ...
        $dompdf->setPaper('A4', 'portrait');
        $dompdf->render();
        $dompdf->stream("LeaveRegister-" . date('Y-m', strtotime($month)) . ".pdf");
    }
}
