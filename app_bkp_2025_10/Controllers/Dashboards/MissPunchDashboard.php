    ...
    public function getMissingPunchingReport()
    {
        ...
        if (!empty($AllEmployees)) {
            foreach ($AllEmployees as $employee_row) {
                ...
                $data_all_days = Processor::getProcessedDashboardData($employee_row['employee_id'], $first_date_of_month, $current_date_of_month);
                if (!empty($data_all_days)) {
                    foreach ($data_all_days as $i => $data_all_day_row) {
                        if ((!empty($data_all_day_row['punch_in_time']) && empty($data_all_day_row['punch_out_time'])) || empty($data_all_day_row['punch_in_time']) && !empty($data_all_day_row['punch_out_time'])) {
                            if ($data_all_day_row['date'] != date('Y-m-d')) {
                                $data_all_days[$i]['missed_punch'] = 'yes';
                                if ($data_all_day_row['is_onOD'] == 'yes') {
                                    $data_all_days[$i]['missed_punch_on_od'] = 'yes';
                                } else {
                                    $data_all_days[$i]['missed_punch_on_od'] = 'no';
                                }
                            } else {
                                $data_all_days[$i]['missed_punch'] = 'no';
                                $data_all_days[$i]['missed_punch_on_od'] = 'no';
                            }
                        } else {
                            $data_all_days[$i]['missed_punch'] = 'no';
                ...
                $missed_punch_on_od_mtd = 0;
                $missed_punch_on_od_last_7day = 0;
                $missed_punch_on_od_last_15day = 0;
                ...
                if (!empty($data_all_days)) {
                    ...
                    foreach ($data_all_days as $data_all_days_row) {
                        if (strtotime($data_all_days_row['date']) >= strtotime($first_date_of_month) && strtotime($data_all_days_row['date']) <= strtotime($current_date_of_month)) {
                            if ($data_all_days_row['missed_punch'] == 'yes') {
                                $missed_punch_mtd += 1;
                            }
                            if ($data_all_days_row['missed_punch_on_od'] == 'yes') {
                                $missed_punch_on_od_mtd += 1;
                            }
                        }
                        if (strtotime($data_all_days_row['date']) >= strtotime($last_7_days_date) && strtotime($data_all_days_row['date']) <= strtotime($current_date_of_month)) {
                            if ($data_all_days_row['missed_punch'] == 'yes') {
                                $missed_punch_last_7day += 1;
                            }
                            if ($data_all_days_row['missed_punch_on_od'] == 'yes') {
                                $missed_punch_on_od_last_7day += 1;
                            }
                        }
                        if (strtotime($data_all_days_row['date']) >= strtotime($last_15_days_date) && strtotime($data_all_days_row['date']) <= strtotime($current_date_of_month)) {
                            if ($data_all_days_row['missed_punch'] == 'yes') {
                                $missed_punch_last_15day += 1;
                            }
                            if ($data_all_days_row['missed_punch_on_od'] == 'yes') {
                                $missed_punch_on_od_last_15day += 1;
                            }
                        }
                    }
                }
                ...
                $DataRow['missed_punch_on_od_mtd']  = $missed_punch_on_od_mtd;
                $DataRow['missed_punch_last_7day']  = $missed_punch_last_7day;
                $DataRow['missed_punch_on_od_last_7day']  = $missed_punch_on_od_last_7day;
                $DataRow['missed_punch_last_15day']  = $missed_punch_last_15day;
                $DataRow['missed_punch_on_od_last_15day']  = $missed_punch_on_od_last_15day;
