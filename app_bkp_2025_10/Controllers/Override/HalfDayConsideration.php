...
use App\Models\WaveOffHalfDayWhoDidNotWorkForHalfDayRevisionModel;

class HalfDayConsideration extends BaseController
{
    public $session;
    ...
    public function index()
    {
        ...
        $data = [
            'page_title'            => 'Wave Off Half Day Who Did Not Work For Half Day',
            'current_controller'    => $this->request->getUri()->getSegment(2),
            'current_method'        => $this->request->getUri()->getSegment(3),
            'employees'             => $this->getAllEmployees(),
        ];

        return view('WaveOffHalfDayWhoDidNotWorkForHalfDay/WaveOffHalfDayWhoDidNotWorkForHalfDay', $data);
    }
    ...
    public function updateWaveOffHalfDayWhoDidNotWorkForHalfDay()
    {
        ...
        if (!$validation) {
            ...
            $WaveOffHalfDayWhoDidNotWorkForHalfDayModel->where('employee_id=', $employee_id)->where('date', $wave_off_date);
            $existing_wave_off_entry = $WaveOffHalfDayWhoDidNotWorkForHalfDayModel->first();
            ...
            if (!empty($existing_wave_off_entry)) {
                ...
                $WaveOffHalfDayWhoDidNotWorkForHalfDayModel = new WaveOffHalfDayWhoDidNotWorkForHalfDayModel();
                $wave_off_entry_update_query = $WaveOffHalfDayWhoDidNotWorkForHalfDayModel->update($existing_wave_off_entry_id, $data);
                $last_wave_off_entry_id = $existing_wave_off_entry_id;
            } else {
                $data = [
                    'employee_id' => $employee_id,
                    'date' => $wave_off_date,
                ...
                $WaveOffHalfDayWhoDidNotWorkForHalfDayModel = new WaveOffHalfDayWhoDidNotWorkForHalfDayModel();
                $wave_off_entry_update_query = $WaveOffHalfDayWhoDidNotWorkForHalfDayModel->insert($data);
                $last_wave_off_entry_id = $WaveOffHalfDayWhoDidNotWorkForHalfDayModel->getInsertID();
            }
            ...
            if ($wave_off_entry_update_query) {
                ...
                $notification_email->setSubject('Consideration of Half day updated using Overrides by ' . $this->session->get("current_user")["name"]);
                                    ...
                                    <td align="left" valign="center">
                                        <div style="text-align:left; margin: 0 20px; padding: 40px; background-color:#ffffff; border-radius: 6px">
                                            ...
                                            <div style="padding-bottom: 30px; font-size: 17px;">
                                                <strong>Consideration of Half day updated using Overrides by ' . $this->session->get("current_user")["name"] . '</strong>
                                            </div>
                                            ...
                                            <div style="padding-bottom: 10px">details can be found in WaveOffHalfDayWhoDidNotWorkForHalfDayModel at id ' . $last_wave_off_entry_id . '</div>
    ...
    /*public function GetAllWaveOffHalfDayWhoDidNotWorkForHalfDay(){
        $WaveOffHalfDayWhoDidNotWorkForHalfDayModel = new WaveOffHalfDayWhoDidNotWorkForHalfDayModel();
        $WaveOffHalfDayWhoDidNotWorkForHalfDayModel
        ->select('waveoffhalfdaywhodidnotworkforhalfday.*')
        ->select('employees.id as employee_id')
        ->select("trim(concat(employees.first_name, ' ', employees.last_name)) as employee_name")
        ->select('employees.internal_employee_id as internal_employee_id')
        ->select('companies.company_short_name as company_short_name')
        ->select('departments.department_name as department_name')
        ->join('employees', 'employees.id=waveoffhalfdaywhodidnotworkforhalfday.employee_id', 'left')
        ->join('companies as companies', 'companies.id = employees.company_id', 'left')
        ->join('departments as departments', 'departments.id = employees.department_id', 'left')
        ->orderBy('employees.id', 'ASC');
        $allWaveOffHalfDayWhoDidNotWorkForHalfDayEntries = $WaveOffHalfDayWhoDidNotWorkForHalfDayModel->findAll();

        $data = [
            'page_title'            => 'All Wave Off Minutes',
            'current_controller'    => $this->request->getUri()->getSegment(2),
            'current_method'        => $this->request->getUri()->getSegment(3),
            'allWaveOffHalfDayWhoDidNotWorkForHalfDayEntries' => $allWaveOffHalfDayWhoDidNotWorkForHalfDayEntries,
        ];
        return view('WaveOffMinutes/WaveOffMinutesAll', $data);
    }*/

    public function existingWaveOffHalfDayWhoDidNotWorkForHalfDay()
    {
        ...
        $WaveOffHalfDayWhoDidNotWorkForHalfDayModel
            ...
            ->join('companies as companies', 'companies.id = employees.company_id', 'left')
            ->join('departments as departments', 'departments.id = employees.department_id', 'left')
            ->where('waveoffhalfdaywhodidnotworkforhalfday.employee_id =', $employee_id)
            ->orderBy('employees.id', 'ASC');
        $allWaveOffHalfDayWhoDidNotWorkForHalfDayEntries = $WaveOffHalfDayWhoDidNotWorkForHalfDayModel->findAll();

        if (!empty($allWaveOffHalfDayWhoDidNotWorkForHalfDayEntries)) {
            foreach ($allWaveOffHalfDayWhoDidNotWorkForHalfDayEntries as $i => $d) {
                $allWaveOffHalfDayWhoDidNotWorkForHalfDayEntries[$i]['date'] = date('d M, Y', strtotime($d['date']));
            }
        }

        echo json_encode($allWaveOffHalfDayWhoDidNotWorkForHalfDayEntries);
    }

    public function DeleteWaveOffHalfDayWhoDidNotWorkForHalfDay()
    {
        $response_array = array();
        $validation = $this->validate(
            [
                'override_id'  =>  [
                    'rules'         =>  'required|is_not_unique[waveoffhalfdaywhodidnotworkforhalfday.id]',
                    'errors'        =>  [
        ...
        if (!$validation) {
            ...
            $WaveOffHalfDayWhoDidNotWorkForHalfDayModel = new WaveOffHalfDayWhoDidNotWorkForHalfDayModel();
            #save revision
            $old_data = $WaveOffHalfDayWhoDidNotWorkForHalfDayModel->find($override_id);
            $old_data['override_id'] = $old_data['id'];
            unset($old_data['id']);
            $old_data['revised_by'] = $this->session->get('current_user')['employee_id'];

            $WaveOffHalfDayWhoDidNotWorkForHalfDayRevisionModel = new WaveOffHalfDayWhoDidNotWorkForHalfDayRevisionModel();
            $revision_query = $WaveOffHalfDayWhoDidNotWorkForHalfDayRevisionModel->insert($old_data);
            if (!$revision_query) {
                $response_array['response_type'] = 'error';
                $response_array['response_description'] = 'Revision Error <br> Please contact administrator.';
                $response_array['response_data'] = $old_data;
                $response_array['response_error'] = $WaveOffHalfDayWhoDidNotWorkForHalfDayRevisionModel->error();
            } else {
                $last_wave_off_entry_id = $WaveOffHalfDayWhoDidNotWorkForHalfDayRevisionModel->getInsertID();
                #delete
                $WaveOffHalfDayWhoDidNotWorkForHalfDayModel = new WaveOffHalfDayWhoDidNotWorkForHalfDayModel();
                $query = $WaveOffHalfDayWhoDidNotWorkForHalfDayModel->delete($override_id);
                if (!$query) {
                    $response_array['response_type'] = 'error';
                    ...
                    $to_emails = array('developer3@healthgenie.in');
                    $notification_email->setTo($to_emails);
                    $notification_email->setSubject('Consideration of Half day deleted using Overrides by ' . $this->session->get("current_user")["name"]);
                    $notification_email->setMessage(
                        '<div style="font-family:Arial,Helvetica,sans-serif; line-height: 1.5; font-weight: normal; font-size: 15px; color: #2F3044; min-height: 100%; margin:0; padding:0; width:100%; background-color:#edf2f7">
                            <table align="center" border="0" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;margin:0 auto; padding:0; max-width:600px">
                                <tbody>
                                    ...
                                    <tr>
                                        <td align="left" valign="center">
                                            <div style="text-align:left; margin: 0 20px; padding: 40px; background-color:#ffffff; border-radius: 6px">
                                                <!--begin:Email content-->
                                                <div style="padding-bottom: 30px; font-size: 17px;">
                                                    <strong>Consideration of Half day deleted using Overrides by ' . $this->session->get("current_user")["name"] . '</strong>
                                                </div>
                                                <div style="padding-bottom: 10px">Details of the leave request are mentioned below.</div>
                                                ...
                                                <div style="padding-bottom: 10px">details can be found in WaveOffHalfDayWhoDidNotWorkForHalfDayRevisionModel at id ' . $last_wave_off_entry_id . '</div>
