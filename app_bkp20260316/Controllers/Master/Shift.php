...
class Shift extends BaseController
{
    ...
    public function addShift()
    {
        ...
        if (!$validation) {
            ...
            $values = [
                ...
                'shift_type'   => $this->request->getPost('shift_type') ?? 'regular',
    ...
    public function getShift()
    {
        ...
        if (!$validation) {
            ...
            $the_shift_response['shift_type']['value'] = $the_shift['shift_type'];
            $the_shift_response['shift_type']['data_value'] = $the_shift['shift_type'];
        ...
        if (!$validation) {
            ...
            if (!$query) {
                ...
                if (strtotime($this->request->getPost('shift_start_monday_edit')) > strtotime($this->request->getPost('shift_end_monday_edit'))) {
                    ...
                    // $half_day_for_work_hours_edit = $diff_hrs_edit . ":" . $diff_minutes_edit;
                    // $half_day_for_work_hours_edit = sprintf('%02d:%02d', $diff_hrs_edit, $diff_minutes_edit);
                    $half_day_for_work_hours_edit = str_pad($diff_hrs_edit, 2, '0', STR_PAD_LEFT) . ":" . str_pad($diff_minutes_edit, 2, '0', STR_PAD_LEFT);
                }
