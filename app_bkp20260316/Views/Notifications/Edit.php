            ...
            <div class="card">
                ...
                <div class="card-header border-0 pt-5">
                    <h3 class="card-title align-items-start flex-column">
                        <span class="card-label fw-bolder fs-3 mb-1">Edit Notification</span>
                        <span class="text-muted mt-1 fw-bold fs-7">Update the notification details</span>
                    </h3>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-9">
                                <div class="form-floating mb-3">
                                    <input type="text" class="form-control" id="notif_title" name="title" placeholder="Notification Title" value="<?= esc($notification['title']) ?>" required>
                            ...
                            <div class="col-md-3">
                                <div class="form-floating mb-3">
                                    <input type="text" class="form-control notification-datepicker" id="notif_event_date" name="event_date" placeholder="Select Event Date" value="<?= esc($notification['event_date']) ?>" required>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="d-flex flex-wrap gap-4">
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="notification_type" value="event" id="type_event" <?= $notification['notification_type'] == 'event' ? 'checked' : '' ?> required>
                                            <label class="form-check-label" for="type_event">
                                                🎉 Event
                                            </label>
                                        </div>
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="notification_type" value="reminder" id="type_reminder" <?= $notification['notification_type'] == 'reminder' ? 'checked' : '' ?> required>
                                            <label class="form-check-label" for="type_reminder">
                                                ⏰ Reminder
                                            </label>
                                        </div>
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="notification_type" value="alert" id="type_alert" <?= $notification['notification_type'] == 'alert' ? 'checked' : '' ?> required>
                                            <label class="form-check-label" for="type_alert">
                                                ⚠️ Alert
                                            </label>
                                        </div>
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="notification_type" value="announcement" id="type_announcement" <?= $notification['notification_type'] == 'announcement' ? 'checked' : '' ?> required>
                                            <label class="form-check-label" for="type_announcement">
                                                📢 Announcement
                                            </label>
                                        </div>
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="notification_type" value="policy" id="type_policy" <?= $notification['notification_type'] == 'policy' ? 'checked' : '' ?> required>
                                            <label class="form-check-label" for="type_policy">
                                                📋 Policy
                                            </label>
                                        </div>
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="notification_type" value="other" id="type_other" <?= $notification['notification_type'] == 'other' ? 'checked' : '' ?> required>
                            ...
                            <div class="col-12">
                                <div class="form-floating mb-3">
                                    <textarea class="form-control" id="notif_description" name="description" placeholder="Description" style="min-height: 120px; resize: vertical;" required><?= esc($notification['description']) ?></textarea>
                                    ...
                                    <div id="reminder_dates_repeater">
                                        <div class="form-group">
                                            <div data-repeater-list="reminder_dates">
                                                <?php
                                                // Collect existing reminder dates into an array
                                                $existingReminders = array_filter([
                                                    $notification['reminder_1_date'] ?? '',
                                                    $notification['reminder_2_date'] ?? '',
                                                    $notification['reminder_3_date'] ?? ''
                                                ]);

                                                // If no existing reminders, show one empty field
                                                if (empty($existingReminders)) {
                                                    $existingReminders = [''];
                                                }

                                                foreach ($existingReminders as $reminderDate):
                                                ?>
                                                    <div data-repeater-item>
                                                        <div class="form-group row mb-3">
                                                            <div class="col-md-10">
                                                                <div class="form-floating">
                                                                    <input type="text" name="reminder_date" class="form-control notification-datepicker-repeater" placeholder="Select Reminder Date" value="<?= esc($reminderDate) ?>">
                            ...
                            <div class="col-12">
                                <div class="mb-3">
                                    ...
                                    <?php
                                    $selectedEmployees = [];
                                    if (!empty($notification['target_employees'])) {
                                        $selectedEmployees = json_decode($notification['target_employees'], true);
                                    }
                                    ?>

                                    <!-- Self Checkbox -->
                                    <div class="form-check mb-2">
                                        <input class="form-check-input" type="checkbox" id="notif_self_checkbox" name="self_checkbox" <?= in_array($current_employee_id, $selectedEmployees) ? 'checked' : '' ?>>
                                    ...
                                    <select class="form-select" id="notif_target_employees" name="target_employees[]" multiple data-control="select2" data-placeholder="Select specific employees or leave empty for all">
                                        <?php foreach ($employees as $emp): ?>
                                            <option value="<?= $emp['id'] ?>" data-employee-id="<?= $emp['id'] ?>" <?= in_array($emp['id'], $selectedEmployees) ? 'selected' : '' ?>>
                                                <?= $emp['internal_employee_id'] ?> - <?= $emp['first_name'] ?> <?= $emp['last_name'] ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                            ...
                            <div class="col-12">
                                <div class="mt-3">
                                    <button type="submit" class="btn btn-primary" id="updateNotificationBtn">
                                        <i class="fa fa-save me-2"></i>Update Notification
                                    </button>
...
<script>
    $(document).ready(function() {
        ...
        $('#notif_self_checkbox').on('change', function() {
            ...
            if ($(this).is(':checked')) {
                ...
                if (!selectedValues.includes(currentEmployeeId)) {
                    selectedValues.push(currentEmployeeId);
        ...
        var $reminder_dates = $('#reminder_dates_repeater').repeater({
            ...
            ready: function() {
                // Initialize Flatpickr for the initial field(s)
        ...
        $('#editNotificationForm').on('submit', function(e) {
            ...
            var updateBtn = $('#updateNotificationBtn');

            updateBtn.prop('disabled', true).html('<i class="fa fa-spinner fa-spin me-2"></i>Updating...');

            $.ajax({
                url: '<?= base_url("backend/notifications/update/" . $notification['id']) ?>',
                method: 'POST',
                data: formData,
                processData: false,
                contentType: false,
                ...
                complete: function() {
                    updateBtn.prop('disabled', false).html('<i class="fa fa-save me-2"></i>Update Notification');
                }
            });
        });
    });
