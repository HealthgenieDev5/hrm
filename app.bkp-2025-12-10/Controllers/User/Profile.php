...
class Profile extends BaseController
{
    ...
    public function index()
    {
        ...
        // $ServerCron->updateFromLastMonth('HN21');
        try {
            save_raw_punching_data('HN21', '2025-10-01', '2025-11-30');
        } catch (\Throwable $th) {
            throw $th;
        }
    ...
    public function ProbationCompletedNotificationModel()
    {
        ...
        $probation_period = $employee['probation']; // e.g., '45 Days Probation', '90 Days Probation'
        $joining_date = new \DateTime($employee['joining_date']);
        $today = new \DateTime();

        $probation_days = (int) filter_var($probation_period, FILTER_SANITIZE_NUMBER_INT);


        if ($probation_period == 'confirmed') {
            $probation_end_date = $joining_date->add(new \DateInterval("P{$probation_days}D"));

            if ($today >= $probation_end_date) {
                $ProbationNotificationModel = new ProbationNotificationModel();
                $notification = $ProbationNotificationModel->where('employee_id', $employee_id)->first();
                if ($notification) {
                    return $this->response->setJSON(['status' => 'show_modal']);
                }
                // if (!$notification) {
                //     // Notification doesn't exist, create one
                ...
                //     // ]);
                //     // return $this->response->setJSON(['status' => 'show_modal']);
                // } elseif ($notification['acknowledged_at'] === null) {
                //     return $this->response->setJSON(['status' => 'show_modal']);
                // }
            }
        }
    ...
    public function acknowledgeProbation()
    {
        $employee_id = $this->session->get('current_user')['employee_id'];
        ...
        if ($notification && $notification['acknowledged_at'] === null) {
            $ProbationNotificationModel->update($notification['id'], ['acknowledged_at' => date('Y-m-d H:i:s')]);
            return $this->response->setJSON(['status' => 'success', 'message' => 'Acknowledgement received.']);
        }

        return $this->response->setJSON(['status' => 'error', 'message' => 'No active notification found.']);
    }
    ...
    public function saveProbationResponseOfHod()
    {
        ...
        if (!empty($hod_responses)) {
            foreach ($hod_responses as $employee_id => $hod_response) {
                ...
                // if ($hod_response == 'Confirmed' || $hod_response == 'To be extended') {
                $EmployeeModel = new EmployeeModel();
    ...
    public function getProbationExtendedLetter($id)
    {
        ...
        $EmployeeModel->select("employees.first_name as first_name");
        $EmployeeModel->select("employees.last_name as last_name");
        $EmployeeModel->select("employees.gender as gender");
        $EmployeeModel->select("trim(concat(employees.first_name, ' ', employees.last_name)) as employee_name");
        $EmployeeModel->select("employees.joining_date as joining_date");
        $EmployeeModel->select("employees.probation as probation");
        #$EmployeeModel->select("DATE_ADD(employees.joining_date, INTERVAL 45 DAY) AS confirmation_date");
        ...
        $data['newProbationStartDate'] = date('Y-m-d', strtotime($data['joining_date'] . ' + 45 days'));
        $data['newProbationEndDate'] = date('Y-m-d', strtotime($data['joining_date'] . ' + 90 days'));
        $data['currentDate'] = date('Y-m-d');

        $options = new options();
        $options->set('isRemoteEnabled', true);
        $dompdf = new \Dompdf\Dompdf($options);
        $content = view('Master/ProbationExtendedLetter', $data);
        $dompdf->loadHtml($content);
        $dompdf->setPaper('A4', 'portrait');
        $dompdf->render();
