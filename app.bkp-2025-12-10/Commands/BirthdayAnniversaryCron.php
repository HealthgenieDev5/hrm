...
class BirthdayAnniversaryCron extends BaseCommand
{
    ...
    protected $options = [
        '--monthly' => 'Generate notifications for the entire month (optionally specify YYYY-MM format, e.g., --monthly=2025-11)',
    ];

    public function run(array $params)
    {
        // Parse monthly option
        $yearMonth = null;
        $isMonthlyMode = false;

        // Check if monthly flag exists in params
        if (isset($params['monthly'])) {
            $isMonthlyMode = true;
            $yearMonth = $params['monthly'] ?: null;
        }

        // Check for combined format like 'monthly=2025-12' in keys
        foreach (array_keys($params) as $key) {
            if (strpos($key, 'monthly=') === 0) {
                $isMonthlyMode = true;
                $yearMonth = substr($key, strlen('monthly='));
                break;
            } elseif ($key === 'monthly') {
                $isMonthlyMode = true;
                break;
