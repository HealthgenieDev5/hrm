...
<!--begin::System Announcement Modal-->
<!-- <div class="modal fade" id="sysAnnModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header" id="sysAnnHdr">
                <h5 class="modal-title"><i class="fas fa-bullhorn me-2"></i><span id="sysAnnTitle"></span></h5>
                <span class="badge bg-secondary" id="sysAnnCount"></span>
</div>
<div class="modal-body" id="sysAnnMsg"></div>
<div class="modal-footer">
    <button type="button" class="btn btn-secondary" id="sysAnnPrev" style="display:none;"><i class="fas fa-chevron-left"></i> Previous</button>
    <button type="button" class="btn btn-secondary" id="sysAnnNext" style="display:none;">Next <i class="fas fa-chevron-right"></i></button>
    <button type="button" class="btn btn-primary" id="sysAnnAck"><i class="fas fa-check-circle me-1"></i> I Acknowledge</button>
</div>
</div>
</div>
</div> -->
<!--end::System Announcement Modal-->
...
<script>
    ...
    // Old static announcement popup - replaced by dynamic system
    // function showAnnouncementPopup() {
    //     setTimeout(function() {
    //         if ($('#announcementPopup').length) {
...
<script type="text/javascript">
    ...
    $(document).on('click', '#sysAnnAck', function() {
        handleAcknowledgeAnnouncement();
    });

    $(document).on('click', '#sysAnnNext', function() {
        if (currentIdx < announcements.length - 1) showAnnouncementModal(currentIdx + 1);
    });

    $(document).on('click', '#sysAnnPrev', function() {
        if (currentIdx > 0) showAnnouncementModal(currentIdx - 1);
    });

    function checkAnnouncementPopup() {
        setTimeout(function() {
            $.ajax({
                url: '<?= base_url('announcement/get-pending') ?>',
                method: 'GET',
                success: function(response) {
                    if (response.success && response.count > 0) {
                        announcements = response.data;
                        currentIdx = 0;
                        showAnnouncementModal(0);
                    }
                },
                error: function() {
    ...
    function showAnnouncementModal(idx) {
        var a = announcements[idx];
        currentIdx = idx;

        $('#sysAnnTitle').text(a.title);
        $('#sysAnnMsg').html(a.message);
        $('#sysAnnCount').text((idx + 1) + '/' + announcements.length);

        var hdr = $('#sysAnnHdr').removeClass('bg-warning bg-danger bg-success bg-primary text-white text-dark');
        ...
        $('#sysAnnPrev').toggle(idx > 0 && announcements.length > 1);
        $('#sysAnnNext').toggle(idx < announcements.length - 1 && announcements.length > 1);

        $("#sysAnnModal").modal('show');
    }
    ...
    function handleAcknowledgeAnnouncement() {
        ...
        $.ajax({
            url: '<?= base_url('announcement/acknowledge') ?>',
            method: 'POST',
            data: {
                announcement_id: announcements[currentIdx].id,
                '<?= csrf_token() ?>': '<?= csrf_hash() ?>'
            },
            success: function(response) {
                if (response.success) {
                    announcements.splice(currentIdx, 1);
                    if (announcements.length > 0) {
                        if (currentIdx >= announcements.length) currentIdx = announcements.length - 1;
                        showAnnouncementModal(currentIdx);
                    } else {
                        $("#sysAnnModal").modal('hide');
                    }
                } else {
                    alert(response.message || 'Failed to acknowledge announcement');
                }
                btn.prop('disabled', false).html('<i class="fas fa-check-circle me-1"></i> I Acknowledge');
            },
            error: function() {
