...
if (! function_exists('get_raw_punching_data')) {
    function get_raw_punching_data($employee_id = 'ALL', $from_date = '', $to_date = '')
    {
        ...
        $InOutPunchData = [];
        $attendance_api_url = env('ATTENDANCE_API_URL');
        $attendance_api_key = env('ATTENDANCE_API_KEY');
        $attendance_api_secret = env('ATTENDANCE_API_SECRET');

        if (!empty($attendance_api_url) && !empty($attendance_api_key) && !empty($attendance_api_secret)) {
            $url = $attendance_api_url . '/attendance/raw';
            $query_params = [
                'from_date' => $from_date,
                'to_date' => $to_date,
            ];
            ...
            if ($employee_id !== 'ALL') {
                $query_params['emp_code'] = $employee_id;
            }
            ...
            $headers = [
                ...
                'X-API-SECRET: ' . $attendance_api_secret,
            ];

            $curl = curl_init($full_url);
            curl_setopt($curl, CURLOPT_CUSTOMREQUEST, 'GET');
            curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, false); // For local development, consider removing in production
            curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false); // For local development, consider removing in production
        ...
        // Use API data if available, otherwise fall back to empty
        $InOutPunchData__Del__GGN_Noida = $InOutPunchData ?? [];
        ...
        if (!empty($InOutPunchData__Del__GGN_Noida)) {
            ...
            foreach ($InOutPunchData__Del__GGN_Noida as $i => $d) {
                ...
                // API already provides DateString_2 in correct format - don't overwrite it
