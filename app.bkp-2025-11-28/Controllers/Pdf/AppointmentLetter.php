...
class AppointmentLetter extends BaseController
{
	public $session;
	public $uri;
	...
	public function index($id)
	{
		...
		// return view('Master/EmployeeAppointmentLetter', $data); die();

		$options = new options();
		$options->set('isRemoteEnabled', true);
		$dompdf = new \Dompdf\Dompdf($options);
		$content = view('Master/EmployeeAppointmentLetter', $data);
		$dompdf->loadHtml($content);
		$dompdf->setPaper('A4', 'portrait');
		$dompdf->render();
		$dompdf->stream("appointment-letter-" . strtolower($data['first_name'] . '-' . $data['last_name']) . "-.pdf");
	}
}
